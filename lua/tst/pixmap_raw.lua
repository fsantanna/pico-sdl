...
do
    ...
    pico.output.clear()
    local r = {'!', x=1, y=9, anchor='NW'}
    pico.output.draw.pixmap("buf2", pixmap, r)
    pico.check("pixmap-02")
end

do
    print("aspect: w=6 only on 3x3 -> 6x6")
    pico.set.show { color='black' }
    pico.output.clear()
    local r = {'!', x=1, y=1, w=6, anchor='NW'}
    pico.output.draw.pixmap("buf3", pixmap, r)
    pico.check("pixmap-03")
end

do
    print("distort: w=6, h=3 -> x-stretched")
    pico.set.show { color='white' }
    pico.output.clear()
    local r = {'!', x=1, y=1, w=6, h=3, anchor='NW'}
