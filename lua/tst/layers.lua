...
pico.set.layer(bg)
pico.set.color.clear(0x80, 0x00, 0x00)
pico.output.clear()
pico.output.draw.rect({'%', x=0.5, y=0.5, w=0.5, h=0.5, anc='C'})
pico.set.layer()
pico.output.clear()
pico.output.draw.layer(bg, {'%', x=0.5, y=0.5, w=1, h=1, anc='C'})
pico.check("layers-01")

-- draw on ui layer (blue background)
print("draw on ui layer")
pico.set.layer(ui)
pico.set.color.clear(0x00, 0x00, 0x80)
pico.output.clear()
pico.set.color.draw(0x00, 0xFF, 0x00)
pico.output.draw.rect({'%', x=0.5, y=0.5, w=0.5, h=0.5, anc='C'})
pico.set.layer(nil)
pico.output.clear()
pico.output.draw.layer(ui, {'%', x=0.5, y=0.5, w=1, h=1, anc='C'})
...
pico.set.color.clear(0x00, 0x00, 0x00)
pico.output.clear()
pico.output.draw.layer(bg, {'%', x=1.0/3, y=1.0/3, w=1.0/3, h=1.0/3, anc='C'})
pico.output.draw.layer(ui, {'%', x=2.0/3, y=2.0/3, w=1.0/3, h=1.0/3, anc='C'})
