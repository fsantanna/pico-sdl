...
print "pico.vs.pos_rect (cur vs cur)"
do
    print('', 1)
    local r = {'!', x=25, y=25, w=50, h=50, anchor='NW'}
    local p = {'!', x=50, y=50, anchor='NW'}
    assert(pico.vs.pos_rect(nil, p, nil, r))

    print('', 2)
    local r = {'!', x=25, y=25, w=50, h=50, anchor='NW'}
    local p = {'!', x=10, y=10, anchor='NW'}
    assert(not pico.vs.pos_rect(nil, p, nil, r))
end

print "pico.vs.rect_rect (cur vs cur)"
do
    print('', 1)
    local r1 = {'!', x=20, y=20, w=40, h=40, anchor='NW'}
    local r2 = {'!', x=30, y=30, w=40, h=40, anchor='NW'}
    assert(pico.vs.rect_rect(nil, r1, nil, r2))

    print('', 2)
    local r1 = {'!', x=10, y=10, w=30, h=30, anchor='NW'}
    local r2 = {'!', x=50, y=50, w=30, h=30, anchor='NW'}
    assert(not pico.vs.rect_rect(nil, r1, nil, r2))
end

print "pico.vs.pos_pos (cur vs cur)"
do
    print('', 1)
    local p1 = {'!', x=50, y=50, anchor='NW'}
    local p2 = {'%', x=0.5, y=0.5, anchor='NW'}
    assert(pico.vs.pos_pos(nil, p1, nil, p2))

    print('', 2)
    local p1 = {'!', x=50, y=50, anchor='NW'}
    local p2 = {'!', x=51, y=50, anchor='NW'}
    assert(not pico.vs.pos_pos(nil, p1, nil, p2))
end

print "child layer projection"
do
