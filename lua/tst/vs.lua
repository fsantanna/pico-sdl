...
do
    ...
    assert(    pico.vs.pos_pos(p1, p2))                   -- 2-arg
    assert(not pico.vs.pos_pos(p1, q))
    assert(    pico.vs.pos_pos(nil, p1, nil, p2))         -- explicit nils
    assert(not pico.vs.pos_pos(nil, p1, nil, q))
    assert(    pico.vs.pos_pos(nil, p1, p2))              -- trailing L2 omitted
    assert(not pico.vs.pos_pos(nil, p1, q))
    -- p1 in sub_vs (50x50 in cur 100x100, scale 2x) -> cur (20,40)
    local s1 = {'!', x=10, y=20, anchor='NW'}
    local s2 = {'!', x=20, y=40, anchor='NW'}
    assert(    pico.vs.pos_pos('sub_vs', s1, s2))         -- (L1, v1, v2)
    assert(not pico.vs.pos_pos('sub_vs', s1, q))
end
...
do
    ...
    assert(    pico.vs.pos_rect(p_in))                    -- r2 default = cur bounds
    assert(not pico.vs.pos_rect(p_out))
    assert(    pico.vs.pos_rect(p_in,  nil))
    assert(not pico.vs.pos_rect(p_out, nil))
    assert(    pico.vs.pos_rect(nil, p_in,  nil, nil))
    assert(not pico.vs.pos_rect(nil, p_out, nil, nil))
end
...
do
    ...
    assert(    pico.vs.rect_pos(r, p_in))                 -- (r, p)
    assert(not pico.vs.rect_pos(r, p_out))
    ...
    assert(    pico.vs.rect_pos(nil, nil, p_in))
    assert(not pico.vs.rect_pos(nil, nil, p_out))
    assert(    pico.vs.rect_pos(nil, r, nil, p_in))       -- canonical 4-arg
    assert(not pico.vs.rect_pos(nil, r, nil, p_out))
    assert(    pico.vs.rect_pos('sub_vs', nil, nil, p_in))-- r1 = sub's bounds
    assert(not pico.vs.rect_pos('sub_vs', nil, nil, p_out))
end
...
do
    ...
    assert(pico.vs.rect_rect())
    assert(pico.vs.rect_rect(nil, nil, nil, nil))
    local r_in  = {'!', x=10,  y=10,  w=20, h=20, anchor='NW'}
    local r_out = {'!', x=200, y=200, w=10, h=10, anchor='NW'}
    assert(    pico.vs.rect_rect(r_in))                   -- one rect + cur default
    assert(not pico.vs.rect_rect(r_out))                  -- r outside cur
end

print "argument errors"
do
    assert(not pcall(pico.vs.pos_pos))                    -- missing p1+p2
    assert(not pcall(pico.vs.pos_rect))                   -- missing p1
    assert(not pcall(pico.vs.rect_pos))                   -- missing p2
end
...
do
    ...
    assert(not pico.vs.pos_rect(p_over))
    assert(not pico.vs.pos_rect(p_under))
    ...
    assert(not pico.vs.rect_rect(r_over))
    assert(not pico.vs.rect_rect(r_under))
    -- pos_pos: in-range vs out-of-range
    local p_in = {'%', x=0.5, y=0.5, anchor='NW'}
    assert(not pico.vs.pos_pos(p_in, p_over))
    assert(not pico.vs.pos_pos(p_in, p_under))
    -- rect_pos with p2 out of range
    local r = {'!', x=25, y=25, w=50, h=50, anchor='NW'}
    assert(not pico.vs.rect_pos(r, p_over))
    assert(not pico.vs.rect_pos(r, p_under))
end
