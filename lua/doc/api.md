...
- **pico.cv**
    ...
    - **pico.cv.pos_cur_win**: Projects a position from the current layer's
      logical frame to window physical pixels (applies the layer's
      scene.src/dst view transform).
        - `pico.cv.pos_cur_win (pos: Pos) -> { x: integer, y: integer }`
    - **pico.cv.pos_win_cur**: Unprojects a window physical pixel into the
      current layer's logical frame (inverse of `pos_cur_win`).
        - `pico.cv.pos_win_cur (phy: { x: integer, y: integer }, to: Pos)`
        - `to` must have `mode` (and optionally `anchor`) set; `x`/`y` are filled in.
