...
static Pico_Color c_color_tis(lua_State *L, int i) {
  ...
  if (lua_type(L, i) == LUA_TSTRING) {
    ...
    lua_Integer hex = lua_tointeger(L, i);
    return (Pico_Color){
...
static void L_push_color_a(lua_State *L, Pico_Color_A clr) {
  lua_newtable(L);
  lua_pushinteger(L, clr.r);
  lua_setfield(L, -2, "r");
  lua_pushinteger(L, clr.g);
...
static int l_get_view(lua_State *L) {
  ...
  case PICO_FLIP_NONE:
    flip_str = "none";
    break;
  case PICO_FLIP_HORIZONTAL:
    flip_str = "horizontal";
    break;
  case PICO_FLIP_VERTICAL:
    flip_str = "vertical";
    break;
  case PICO_FLIP_BOTH:
    flip_str = "both";
...
static int l_layer_buffer(lua_State *L) {
  ...
  if (buf_dim.w != dim.w || buf_dim.h != dim.h) {
    return luaL_error(L, "buffer size %dx%d doesn't match dim %dx%d", buf_dim.w,
                      buf_dim.h, dim.w, dim.h);
  }
...
static int l_input_event(lua_State *L) {
  ...
  case PICO_EVENT_KEY_UP: {
    lua_pushstring(L, // . | t | tag
                   (e.type == PICO_EVENT_KEY_DOWN ? "key.dn" : "key.up"));
    lua_setfield(L, -2, "tag"); // . | t
...
static const luaL_Reg ll_all[] = {{"init", l_init},
                                  {"quit", l_quit},
                                  {"pop", l_pop},
                                  {"push", l_push},
                                  {NULL, NULL}};
...
static const luaL_Reg ll_cv[] = {
    {"pos", l_cv_pos}, {"rect", l_cv_rect}, {NULL, NULL}};

static const luaL_Reg ll_vs[] = {
    {"pos_rect", l_vs_pos_rect}, {"rect_rect", l_vs_rect_rect}, {NULL, NULL}};

static const luaL_Reg ll_color[] = {{"alpha", l_color_alpha},
...
static const luaL_Reg ll_get[] = {
    {"font", l_get_font},     {"image", l_get_image},
    {"layer", l_get_layer},   {"mouse", l_get_mouse},
    {"text", l_get_text},     {"ticks", l_get_ticks},
    {"video", l_get_video},   {"view", l_get_view},
    {"window", l_get_window}, {NULL, NULL}};

///////////////////////////////////////////////////////////////////////////////

static const luaL_Reg ll_set[] = {
    {"alpha", l_set_alpha},   {"dim", l_set_dim},
    {"expert", l_set_expert}, {"font", l_set_font},
    {"layer", l_set_layer},   {"style", l_set_style},
    {"video", l_set_video},   {"view", l_set_view},
    {"window", l_set_window}, {NULL, NULL}};

static const luaL_Reg ll_set_color[] = {
    {"clear", l_set_color_clear}, {"draw", l_set_color_draw}, {NULL, NULL}};

///////////////////////////////////////////////////////////////////////////////

static const luaL_Reg ll_layer[] = {{"buffer", l_layer_buffer},
                                    {"empty", l_layer_empty},
                                    {"image", l_layer_image},
                                    {"sub", l_layer_sub},
                                    {"text", l_layer_text},
...
static const luaL_Reg ll_input[] = {{"delay", l_input_delay},
                                    {"event", l_input_event},
                                    {"loop", l_input_loop},
                                    {NULL, NULL}};
...
static const luaL_Reg ll_output[] = {{"clear", l_output_clear},
                                     {"present", l_output_present},
                                     {"screenshot", l_output_screenshot},
                                     {"sound", l_output_sound},
                                     {NULL, NULL}};

static const luaL_Reg ll_output_draw[] = {{"buffer", l_output_draw_buffer},
...
int luaopen_pico_native(lua_State *L) {
  ...
  // colors
  {
    ...
    lua_pushlightuserdata(L,
                          (void *)&PICO_COLOR_BLACK); // pico | G | clrs | black
