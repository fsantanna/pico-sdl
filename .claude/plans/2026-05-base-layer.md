...
- [x] **Fix** `pico_output_screenshot` on `pixmap`/`image`/`text` layers — root cause: `SDL_CreateTextureFromSurface` / `IMG_LoadTexture` produce `TEXTUREACCESS_STATIC` textures; `SDL_SetRenderTarget` is a no-op on non-TARGET tex, so `RenderReadPixels` returned the previous target's pixels. **Fix in `src/mem.hc`:** `_alloc_layer_pixmap` / `_alloc_layer_image` / `_tex_text` now use `SDL_CreateTexture(…, RGBA32, TARGET, w, h)` + `SDL_UpdateTexture(…, pixels, pitch)`; image/text additionally `SDL_ConvertSurfaceFormat` so the upload pitch matches RGBA32. All layer textures are now uniformly TARGET-access (consistent with empty layers) — every layer is a writable canvas (incidental win: drawing onto image/text/pixmap layers via `set.layer(L); output.draw.*` now works). `make tests` + `cd lua && make tests` pass. **Pending:** video layer (`STREAMING` tex) screenshot untested — likely also broken; deferred (needs frame-decode setup in test).
