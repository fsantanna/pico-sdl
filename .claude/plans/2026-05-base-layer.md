...
- [x] **Add tests** that exercise `pico_output_screenshot` on **non-window layers** (world, user pixmap/empty/sub) — added 4 cases (`shot-world`, `shot-empty`, `shot-pixmap`, `shot-sub`) inside existing `tst/shot.c` + `lua/tst/shot.lua`; baselines in `tst/asr/`. **Finding:** pixmap-layer screenshots return whatever was last bound as render target (here world's clear color `0x101010`), not the pixmap pixels — `SDL_CreateTextureFromSurface` makes a `STATIC`-access texture, so `SDL_SetRenderTarget(ren, pixmap_tex)` is a no-op and `RenderReadPixels` reads from the previous target. World/empty/sub all work correctly. Test currently locks the buggy pixmap behavior; revisit when fixing screenshot.
