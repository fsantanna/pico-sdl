...
- [x] **Add tests** that exercise `pico_output_screenshot` on **non-window layers** (world, user pixmap/empty/sub/image/text) — 6 cases in `tst/shot.c` + `lua/tst/shot.lua`; baselines in `tst/asr/shot-{world,empty,pixmap,sub,image,text}.png`.
- [x] **Fix screenshot for any non-TARGET layer (pixmap / image / text / video)** — initial approach (commit `4e28dba`) made `pixmap`/`image`/`text` textures TARGET-access in `src/mem.hc`, but couldn't help video (STREAMING/YV12 — TARGET requires RGB-family format). Replaced with a single, universal fallback in `src/pico.c:1661` `pico_output_screenshot`: query tex access; if not TARGET, allocate a temp TARGET tex sized to the layer's `scene.dim`, `SDL_RenderCopy(layer_tex → tmp)` (GPU handles any format conversion incl. YV12→RGB), read from `tmp`, destroy. **Reverted** the `mem.hc` changes — pixmap/image/text are back to STATIC (smaller VRAM, mipmappable). Layer creation semantics unchanged. Both `make tests` (C) and `cd lua && make tests` pass with all 6 non-window screenshot cases (`shot-{world,empty,pixmap,sub,image,text,video}`). Baselines `shot-image.png` and `shot-video.png` regenerated to match the blit-through-TARGET output (pixel-identical to TARGET-tex approach for pixmap/text; image differs slightly because `IMG_LoadTexture` chooses the source format vs the prior explicit `RGBA32` convert).
