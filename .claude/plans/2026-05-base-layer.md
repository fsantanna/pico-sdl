...
- [x] **Add tests** that exercise `pico_output_screenshot` on **non-window layers** (world, user pixmap/empty/sub/image/text) — added 6 cases inside existing `tst/shot.c` + `lua/tst/shot.lua`. Baselines in `tst/asr/shot-{world,empty,pixmap,sub,image,text}.png` reflect **correct** content (generated by temporarily applying TARGET-tex fix to `mem.hc`, then reverting). **Finding:** screenshots are broken on `pixmap`, `image`, `text` layers — all use `SDL_CreateTextureFromSurface` / `IMG_LoadTexture`, which produce `TEXTUREACCESS_STATIC` textures. `SDL_SetRenderTarget(ren, static_tex)` is a no-op, so `RenderReadPixels` reads from the previous target (typically world's clear color). Currently all 3 tests **fail** on `make tests`; world/empty/sub pass. Pending: real fix in `src/mem.hc:122/154/182` (replace with `SDL_CreateTexture(…, TARGET, w, h)` + `SDL_UpdateTexture` + `SDL_ConvertSurfaceFormat` for image/text alpha). Video layer (`STREAMING` tex) untested — likely also broken; deferred.
