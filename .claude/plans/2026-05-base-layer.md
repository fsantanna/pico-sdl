...
- [ ] **Refactor `_pico_output_present` — full traverse-from-window** — replace bespoke world→window.tex blit with `_layer_traverse(&G.window.layer)`. Two prerequisites still pending:
    - **(a) Port aux-clipping into `_pico_output_draw_layer`.** The bespoke blit clips dst to window bounds and propagates the clip into src so over-extending dst rects (`navigate.c`) keep aspect/visible mapping correct. `_pico_output_draw_layer` currently only relies on SDL's automatic dst-clipping, no src compensation.
    ...
    - With both done, `_pico_output_present` collapses to `_layer_traverse(&G.window.layer)` + final mirror, the bespoke blit goes away, and `tst/window.c` window-01 starts passing.
