...
int main (void) {
    ...
    puts("predefined colors");
    {
        ...
        pico_output_draw_rect_pct(&(Pico_Rect_Pct){0.5, 0.1, 0.8, 0.05, C, NULL});

        pico_set_color_draw(PICO_COLOR_GREEN);
        pico_output_draw_rect_pct(&(Pico_Rect_Pct){0.5, 0.2, 0.8, 0.05, C, NULL});

        pico_set_color_draw(PICO_COLOR_BLUE);
        pico_output_draw_rect_pct(&(Pico_Rect_Pct){0.5, 0.3, 0.8, 0.05, C, NULL});

        pico_set_color_draw(PICO_COLOR_YELLOW);
        pico_output_draw_rect_pct(&(Pico_Rect_Pct){0.5, 0.4, 0.8, 0.05, C, NULL});

        pico_set_color_draw(PICO_COLOR_CYAN);
        pico_output_draw_rect_pct(&(Pico_Rect_Pct){0.5, 0.5, 0.8, 0.05, C, NULL});

        pico_set_color_draw(PICO_COLOR_MAGENTA);
        pico_output_draw_rect_pct(&(Pico_Rect_Pct){0.5, 0.6, 0.8, 0.05, C, NULL});

        pico_set_color_draw(PICO_COLOR_ORANGE);
        pico_output_draw_rect_pct(&(Pico_Rect_Pct){0.5, 0.7, 0.8, 0.05, C, NULL});

        pico_set_color_draw(PICO_COLOR_PURPLE);
        pico_output_draw_rect_pct(&(Pico_Rect_Pct){0.5, 0.8, 0.8, 0.05, C, NULL});

        pico_set_color_draw(PICO_COLOR_PINK);
        pico_output_draw_rect_pct(&(Pico_Rect_Pct){0.5, 0.9, 0.8, 0.05, C, NULL});
    ...
    puts("darker function");
    {
        ...
        for (int i=0; i<9; i++) {
            ...
            pico_output_draw_rect_pct (
                &(Pico_Rect_Pct){0.5, 0.1+i*0.1, 0.8, 0.05, PICO_ANCHOR_C, NULL}
            );
        }
...

    {
        ...
        pico_output_draw_rect_pct (
            &(Pico_Rect_Pct){0.5, 0.25, 0.5, 0.15, PICO_ANCHOR_C, NULL}
        );

        // green
        pico_set_color_draw(PICO_COLOR_GREEN);
        pico_output_draw_rect_pct (
            &(Pico_Rect_Pct){0.5, 0.50, 0.5, 0.15, PICO_ANCHOR_C, NULL}
        );

        // -light -> dark
        pico_set_color_draw(pico_color_lighter(PICO_COLOR_GREEN, -0.50));
        pico_output_draw_rect_pct (
            &(Pico_Rect_Pct){0.5, 0.75, 0.5, 0.15, PICO_ANCHOR_C, NULL}
        );
