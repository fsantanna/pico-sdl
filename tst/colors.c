...
int main (void) {
    ...
    puts("predefined colors");
    {
        ...
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.1, 0.8, 0.05}, C, NULL });

        pico_set_pencil_color("root", PICO_COLOR_GREEN);
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.2, 0.8, 0.05}, C, NULL });

        pico_set_pencil_color("root", PICO_COLOR_BLUE);
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.3, 0.8, 0.05}, C, NULL });

        pico_set_pencil_color("root", PICO_COLOR_YELLOW);
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.4, 0.8, 0.05}, C, NULL });

        pico_set_pencil_color("root", PICO_COLOR_CYAN);
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.5, 0.8, 0.05}, C, NULL });

        pico_set_pencil_color("root", PICO_COLOR_MAGENTA);
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.6, 0.8, 0.05}, C, NULL });

        pico_set_pencil_color("root", PICO_COLOR_ORANGE);
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.7, 0.8, 0.05}, C, NULL });

        pico_set_pencil_color("root", PICO_COLOR_PURPLE);
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.8, 0.8, 0.05}, C, NULL });

        pico_set_pencil_color("root", PICO_COLOR_PINK);
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.9, 0.8, 0.05}, C, NULL });
...

    {
        ...
        // -dark -> light
        pico_set_pencil_color("root", pico_color_darker(PICO_COLOR_GREEN, -0.50));
        pico_output_draw_rect("root", 
        ...
        // -light -> dark
        pico_set_pencil_color("root", pico_color_lighter(PICO_COLOR_GREEN, -0.50));
        pico_output_draw_rect("root", 
    ...
    // mirrors Lua test for '%' and '!' color table formats
    {
        ...
        // {'%', r=0, g=1, b=0} -> (0, 255, 0)
        pico_set_pencil_color("root", (Pico_Color){0, 255, 0, 0xFF});
        pico_output_draw_rect("root", 
            &(Pico_Rel_Rect){ '%', {0.5, 0.30, 0.8, 0.1}, C, NULL }
        );

        // {'%', r=0, g=0, b=1} -> (0, 0, 255)
        pico_set_pencil_color("root", (Pico_Color){0, 0, 255, 0xFF});
        pico_output_draw_rect("root", 
            &(Pico_Rel_Rect){ '%', {0.5, 0.45, 0.8, 0.1}, C, NULL }
        );

        // {'%', r=0.5, g=0.5, b=0.5} -> (127, 127, 127)
        pico_set_pencil_color("root", (Pico_Color){127, 127, 127, 0xFF});
        pico_output_draw_rect("root", 
            &(Pico_Rel_Rect){ '%', {0.5, 0.60, 0.8, 0.1}, C, NULL }
        );

        // {'!', r=255, g=255, b=0} -> (255, 255, 0)
        pico_set_pencil_color("root", (Pico_Color){255, 255, 0, 0xFF});
        pico_output_draw_rect("root", 
            &(Pico_Rel_Rect){ '%', {0.5, 0.75, 0.8, 0.1}, C, NULL }
        );

        // {r=0, g=255, b=255} -> (0, 255, 255)
        pico_set_pencil_color("root", (Pico_Color){0, 255, 255, 0xFF});
        pico_output_draw_rect("root", 
    ...
    puts("hex color format");
    {
        ...
        pico_set_pencil_color("root", pico_color_hex(0xFF0000));
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.15, 0.8, 0.1}, C, NULL });

        pico_set_pencil_color("root", pico_color_hex(0x00FF00));
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.30, 0.8, 0.1}, C, NULL });

        pico_set_pencil_color("root", pico_color_hex(0x0000FF));
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.45, 0.8, 0.1}, C, NULL });

        pico_set_pencil_color("root", pico_color_hex(0x7F7F7F));
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.60, 0.8, 0.1}, C, NULL });

        pico_set_pencil_color("root", pico_color_hex(0xFFFF00));
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.75, 0.8, 0.1}, C, NULL });

        pico_set_pencil_color("root", pico_color_hex(0x00FFFF));
        pico_output_draw_rect("root", &(Pico_Rel_Rect){ '%', {0.5, 0.90, 0.8, 0.1}, C, NULL });
    ...
    {
        pico_set_effect_color("root", PICO_COLOR_WHITE);
        pico_output_clear("root");
        Pico_Color pixmap[] = {
    ...
    {
        {
            pico_set_effect_color("root", (Pico_Color){0, 0, 0, 0xFF});
            pico_set_scene_dst (
                "root",
