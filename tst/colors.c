...
int main(void) {
  ...
  pico_set_view(-1, &(Pico_Rel_Dim){'!', {64, 48}, NULL}, NULL, NULL, NULL,
                NULL, NULL, NULL);
  ...
  puts("predefined colors");
  {
    ...
    pico_output_draw_rect(
        &(Pico_Rel_Rect){'%', {0.5, 0.1, 0.8, 0.05}, C, NULL});

    pico_set_color_draw(PICO_COLOR_GREEN);
    pico_output_draw_rect(
        &(Pico_Rel_Rect){'%', {0.5, 0.2, 0.8, 0.05}, C, NULL});

    pico_set_color_draw(PICO_COLOR_BLUE);
    pico_output_draw_rect(
        &(Pico_Rel_Rect){'%', {0.5, 0.3, 0.8, 0.05}, C, NULL});

    pico_set_color_draw(PICO_COLOR_YELLOW);
    pico_output_draw_rect(
        &(Pico_Rel_Rect){'%', {0.5, 0.4, 0.8, 0.05}, C, NULL});

    pico_set_color_draw(PICO_COLOR_CYAN);
    pico_output_draw_rect(
        &(Pico_Rel_Rect){'%', {0.5, 0.5, 0.8, 0.05}, C, NULL});

    pico_set_color_draw(PICO_COLOR_MAGENTA);
    pico_output_draw_rect(
        &(Pico_Rel_Rect){'%', {0.5, 0.6, 0.8, 0.05}, C, NULL});

    pico_set_color_draw(PICO_COLOR_ORANGE);
    pico_output_draw_rect(
        &(Pico_Rel_Rect){'%', {0.5, 0.7, 0.8, 0.05}, C, NULL});

    pico_set_color_draw(PICO_COLOR_PURPLE);
    pico_output_draw_rect(
        &(Pico_Rel_Rect){'%', {0.5, 0.8, 0.8, 0.05}, C, NULL});

    pico_set_color_draw(PICO_COLOR_PINK);
    pico_output_draw_rect(
        &(Pico_Rel_Rect){'%', {0.5, 0.9, 0.8, 0.05}, C, NULL});
  ...
  puts("darker function");
  {
    ...
    for (int i = 0; i < 9; i++) {
      ...
      pico_output_draw_rect(&(Pico_Rel_Rect){
          '%', {0.5, 0.1 + i * 0.1, 0.8, 0.05}, PICO_ANCHOR_C, NULL});
    }
  ...
  puts("hexadecimal integer color format");
  {
    pico_output_clear();
    ...
    pico_output_draw_rect(
        &(Pico_Rel_Rect){'%', {0.5, 0.25, 0.8, 0.1}, C, NULL});

    // 0x00FF00 -> (0, 255, 0)
    pico_set_color_draw(pico_color_hex(0x00FF00));
    pico_output_draw_rect(
        &(Pico_Rel_Rect){'%', {0.5, 0.50, 0.8, 0.1}, C, NULL});
  ...
  puts("color alpha function");
  {
    ...
    pico_output_draw_buffer("alpha", (Pico_Abs_Dim){3, 1}, buffer, &r);
