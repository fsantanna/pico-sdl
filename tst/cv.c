...
int main (void) {
    // EXT - POS - PCT->RAW
    {
        ...
        Pico_Raw_Rect ref  = {25, 25, 50, 50};
        Pico_Pos_Pct  pct = {0.5, 0.5, PICO_ANCHOR_C, NULL};
        Pico_Raw_Pos  raw = pico_cv_pos_pct_raw_ext(&pct, ref);
        printf("pos: (%d, %d)\n", raw.x, raw.y);
    ...
    }
    {
        ...
        Pico_Raw_Rect ref  = {25, 25, 50, 50};
        Pico_Pos      pct = {0.5, 0.5, PICO_ANCHOR_NW, NULL};
        Pico_Raw_Pos  raw = pico_cv_pos_pct_raw_ext(&pct, ref);
        printf("pos: (%d, %d)\n", raw.x, raw.y);
        assert(raw.x==50 && raw.y==50);
    }
    {
        ...
        Pico_Pos     raw = pico_cv_pos_pct_raw_ext(&pct, ref);
        printf("pos: (%d, %d)\n", raw.x, raw.y);
    ...
    // EXT - RECT - PCT->RAW
    {
        ...
        Pico_Raw_Rect  ref  = {20, 20, 60, 60};
        Pico_Rect_Pct pct = {0.25, 0.25, 0.5, 0.25, PICO_ANCHOR_C, NULL};
        Pico_Raw_Rect  raw = pico_cv_rect_pct_raw_ext(&pct, ref);
    ...
    // LOG - RECT - PCT->RAW
    {
        ...
        Pico_Raw_Rect  raw = pico_cv_rect_pct_raw(&pct);
        printf("rect: (%d, %d, %d, %d)\n", raw.x, raw.y, raw.w, raw.h);
