...
int main (void) {
    ...
    pico_set_dim(&dim);
    Pico_Abs_Dim world2;
    world2 = pico_get_scene_dim("root");
    assert(world2.w==window.w && world2.h==window.h);
    pico_set_scene_dim("root", &(Pico_Rel_Dim){ '!', {world.w, world.h}, NULL });
    ...
    for (int i=0; i<50; i++) {
        pico_set_scene_src("root", (Pico_Rel_Rect){ '!', {i, i, 100, 100}, PICO_ANCHOR_NW, NULL });
        pico_output_clear("root");
        pico_output_draw_text("root", "Uma frase bem grande...",
