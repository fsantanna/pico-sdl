...
int main (void) {
    ...
    // pico_output_draw_text_fix: empty string, abs mode -> no crash, screen stays clear
    {
        pico_output_clear();
        Pico_Rel_Rect r = { '!', {10, 10, 0, 10}, PICO_ANCHOR_NW };
        pico_output_draw_text_fix("", r);
        _pico_check("text_empty-01");
    }
    // pico_output_draw_text_fix: empty string, pct mode -> no crash, screen stays clear
    {
        pico_output_clear();
    ...
    {
        pico_output_clear();
        Pico_Rel_Rect r1 = { '!', {10, 10, 0, 10}, PICO_ANCHOR_NW };
        pico_output_draw_text_fix("HELLO", r1);
        Pico_Rel_Rect r2 = { '!', {10, 10, 0, 10}, PICO_ANCHOR_NW };
        pico_output_draw_text_fix("", r2);
