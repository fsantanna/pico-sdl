...
int main (void) {
    ...
    // pico_vs_pos_rect - abs
    {
        ...
        Pico_Rel_Rect rect = { '!', {25, 25, 50, 50}, PICO_ANCHOR_NW };
        Pico_Rel_Pos pos_in = { '!', {50, 50}, PICO_ANCHOR_NW };
    ...
    }
    {
        ...
        Pico_Rel_Rect rect = { '!', {25, 25, 50, 50}, PICO_ANCHOR_NW };
        Pico_Rel_Pos pos_out = { '!', {10, 10}, PICO_ANCHOR_NW };
    ...
    // pico_vs_pos_rect - pct
    {
        ...
        Pico_Rel_Rect rect = { '%', {0.5, 0.5, 0.5, 0.5}, PICO_ANCHOR_C };
        Pico_Rel_Pos pos_in = { '%', {0.5, 0.5}, PICO_ANCHOR_C };
        int result = pico_vs_pos_rect(&pos_in, &rect);
    ...
    }
    {
        ...
        Pico_Rel_Rect rect = { '%', {0.5, 0.5, 0.3, 0.3}, PICO_ANCHOR_C };
        Pico_Rel_Pos pos_out = { '%', {0.1, 0.1}, PICO_ANCHOR_C };
        int result = pico_vs_pos_rect(&pos_out, &rect);
        printf("pos (%.1f, %.1f) vs rect (%.1f, %.1f, %.1f, %.1f): %d\n",
               pos_out.x, pos_out.y, rect.x, rect.y, rect.w, rect.h, result);
        assert(result == 0);
