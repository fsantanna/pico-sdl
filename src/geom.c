...
Pico_Rel_Rect pico_in_rect (Pico_Rel_Rect out, Pico_Rel_Rect in) {
    Pico_Abs_Rect out_abs = _rnd_rect(_raw_rect(out, NULL, NULL));
    Pico_Abs_Rect in_abs  = _rnd_rect(_raw_rect(in, &out_abs, NULL));
...
Pico_Rel_Pos pico_in_pos (Pico_Rel_Rect out, Pico_Rel_Pos in) {
    Pico_Abs_Rect out_abs = _rnd_rect(_raw_rect(out, NULL, NULL));
    Pico_Abs_Pos  in_abs  = _rnd_pos(_raw_pos(in, &out_abs));
...
Pico_Rel_Dim pico_in_dim (Pico_Rel_Rect out, Pico_Rel_Dim in) {
    Pico_Abs_Rect out_abs = _rnd_rect(_raw_rect(out, NULL, NULL));
    Pico_Abs_Dim  in_abs  = _rnd_dim(_raw_dim(&in, &out_abs, NULL));
...
int pico_vs_pos_pos (
    ...
    Pico_Abs_Pos i1 = _rnd_pos(_raw_pos(p1_cur, NULL));
    Pico_Abs_Pos i2 = _rnd_pos(_raw_pos(p2_cur, NULL));
...
int pico_vs_pos_rect (
    ...
    Pico_Abs_Pos  pi = _rnd_pos (_raw_pos(p_cur, NULL));
    Pico_Abs_Rect ri = _rnd_rect(_raw_rect(r_cur, NULL, NULL));
...
int pico_vs_rect_rect (
    ...
    Pico_Abs_Rect i1 = _rnd_rect(_raw_rect(r1_cur, NULL, NULL));
    Pico_Abs_Rect i2 = _rnd_rect(_raw_rect(r2_cur, NULL, NULL));
