...
#define PICO_DIM_PHY { 640, 360, NULL }
#define PICO_DIM_LOG {  64,  36, NULL }
...
typedef enum {
    PICO_ANCHOR_LEFT   = 0,
    PICO_ANCHOR_CENTER = 50,
    PICO_ANCHOR_RIGHT  = 100,
    PICO_ANCHOR_TOP    = 0,
    PICO_ANCHOR_MIDDLE = 50,
    PICO_ANCHOR_BOTTOM = 100,
} PICO_ANCHOR;
...
/// @sa pico_output_draw_image_ext
void pico_output_draw_image (const Pico_Pos* pos, const Pico_Dim* dim, const char* path);

/// @brief Draws a line.
/// @param p1 first point
/// @param p2 second point
void pico_output_draw_line (const Pico_Pos* p1, const Pico_Pos* p2);

/// @brief Draws a single pixel.
/// @param pos drawing coordinate
void pico_output_draw_pixel (const Pico_Pos* pos);
...
void pico_output_draw_rect (const Pico_Pos* pos, const Pico_Dim* dim);

/// @brief Draws a triangle with a right angle at bottom-left.
/// @param rect bounds of the triangle
void pico_output_draw_tri (const Pico_Pos* pos, const Pico_Dim* dim);

/// @brief Draws an ellipse.
/// @param rect bounds of the ellipse
void pico_output_draw_oval (const Pico_Pos* pos, const Pico_Dim* dim);
...
void pico_output_draw_text (const Pico_Pos* pos, const Pico_Dim* dim, const char* text);

void pico_output_draw_fmt (const Pico_Pos* pos, const Pico_Dim* dim, const char* fmt, ...);
...
const char* pico_output_screenshot (Pico_Pos* pos, Pico_Dim* dim, const char* path);
...
void pico_get_clip (Pico_Pos** pos, Pico_Dim** dim);

/// @brief Gets the cropping applied to objects when drawing them.
void pico_get_crop (Pico_Pos** pos, Pico_Dim** dim);
...
void pico_set_clip (const Pico_Pos* pos, const Pico_Dim* dim);
...
void pico_set_crop (const Pico_Pos* pos, const Pico_Dim* dim);

/// @brief Sets the position of the text cursor.
/// @param pos new cursor position
/// @sa pico_output_write
/// @sa pico_output_writeln
void pico_set_cursor (const Pico_Pos* pos);
