...
`pico-sdl` is a C graphics library for developing games and 2D applications.
...
- `pico_input_*` for input events,
    such as waiting key presses and mouse movements.
...
int main (void) {
    ...
    pico_set_size (
        ...
        (Pico_Dim) {16,16}      // logical  screen size (10x10 logical pixel size)
    );
    for (int i=0; i<16; i++) {
...
`pico-sdl` is implemented as a [facade][1] over [SDL][2], which is a more
flexible and lower level API.
This allows `pico-sdl` programmers to fallback to SDL whenever required.
